...
# Change working directory to the directory containing this script to resolve all relative paths correctly
...
def fig_sensitivity():
    ...
    for b,v,m,e in zip(bars,tps,[v/DEMAND for v in tps],err):
        y_pos = v + e + max(tps)*0.035
        ax.text(b.get_x()+b.get_width()/2, y_pos, f"{v:.0f} TPS\n{m:.1f}\u00d7", ha="center", fontsize=8.4, fontweight="bold")
    ...
    for xi,c in zip(x,cpu):
        ax2.annotate(f"{c:.0f}%", (xi,c), textcoords="offset points", xytext=(8,5), fontsize=7.6, color=DEMAND_C)
    annotate_box(ax, 1, max(tps)*1.244, "500 ms = sustained sweet spot;250 ms regresses \n(smaller, costlier blocks)while client CPU stays \u226414% (not saturated)",
...
def fig_gethistory():
    ...
    axR.axhline(200, color=SLATE, ls=":", lw=1.2); axR.text(315, 215, "200 ms interactive threshold", color=SLATE, fontsize=7.8)
    annotate_box(axR, 940, at_1000 + 30, f"\u2248{at_1000:.0f} ms\n@ 1,000 entries", color=NAVY, va="bottom", ha="right")
    axR.annotate(f"crossed at \u2248{thr_at:.0f} entries", xy=(thr_at,200), xytext=(thr_at-100,470),
                 fontsize=7.8, color=RED, arrowprops=dict(arrowstyle="->", color=RED, lw=0.8))

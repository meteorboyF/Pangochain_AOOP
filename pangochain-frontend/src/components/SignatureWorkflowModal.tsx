      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between p-5 border-b border-border">
          <div className="flex items-center gap-2 min-w-0">
            <PenTool className="w-5 h-5 text-[#1d6464] shrink-0" />
            <div className="min-w-0">
              <h2 className="font-heading font-semibold text-text-primary truncate">Signature Workflow</h2>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          {loading ? (
            ...
            <>
              {canInitiate && (
                creating ? (
                  <div className="border border-border rounded-xl p-4 space-y-3 bg-surface-muted/40">
                    <div>
                        ...
                        {members.map((m) => {
                          ...
                          return (
                            <button key={m.userId} onClick={() => toggleSigner(m.userId)}
                              className={`w-full flex items-center gap-2 px-3 py-2 text-left text-sm ${idx >= 0 ? 'bg-[#1d6464]/5' : 'hover:bg-surface-muted'}`}>
                              <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold shrink-0 ${idx >= 0 ? 'bg-[#1d6464] text-white' : 'bg-gray-200 text-gray-500'}`}>
                                {idx >= 0 ? idx + 1 : ''}
                              </span>
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => { setCreating(false); setOrderedSigners([]) }} className="btn border border-border text-text-secondary py-2 flex-1">Cancel</button>
              ...
              {workflows.length === 0 && !creating && (
                <p className="text-text-muted text-sm text-center py-4">No signing workflows on this document.</p>
              )}

              {workflows.map((wf) => (
                <div key={wf.id} className="border border-border rounded-xl p-4">
                  <div className="flex items-center justify-between gap-2">
                    <p className="font-semibold text-text-primary text-sm">{wf.title}</p>
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${wf.status === 'COMPLETED' ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}>{wf.status}</span>
                  </div>

                  <div className="mt-3 space-y-2">
                    {wf.signers.map((s) => (
                      <div key={s.id} className="flex items-center gap-2 text-sm">
                        <span className="w-5 h-5 rounded-full bg-gray-100 text-gray-500 flex items-center justify-center text-[10px] font-bold shrink-0">{s.signOrder + 1}</span>
                        {s.status === 'SIGNED'
                        ...
                        {s.isYourTurn && <span className="flex items-center gap-1 text-[10px] font-bold text-[#1d6464]"><ArrowUp className="w-3 h-3" /> Your turn</span>}
                      </div>
                    ))}
                  </div>
                  ...
                  {wf.signers.some((s) => s.isYourTurn) && (
                    <div className="mt-3 space-y-2">
                      {signingId === wf.id ? (
                        <>
                          <input type="password" className="input" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Password to unlock signing key" />
                          <div className="flex gap-2">
                            <button onClick={() => { setSigningId(null); setPassword('') }} className="btn border border-border text-text-secondary py-1.5 flex-1 text-sm">Cancel</button>
                  ...
                  {wf.status === 'COMPLETED' && (
                    <button onClick={() => downloadCertificate(wf)} className="mt-3 btn border border-[#1d6464] text-[#1d6464] hover:bg-[#1d6464]/10 w-full justify-center py-1.5 text-sm">

...
# ─── 1. Build the chaincode image ────────────────────────────────────────────
log "Building legalcc Docker image..."
docker build -t pangochain/legalcc:latest "$CHAINCODE_DIR"

# ─── 2. Create ccaas connection.json and package ─────────────────────────────
log "Creating ccaas package..."
TMP_PKG=$(mktemp -d)
mkdir -p "${TMP_PKG}/connection"

# Each peer org's chaincode container address (defined in docker-compose.fabric.yml)
cat > "${TMP_PKG}/connection/connection.json" <<'EOF'
{
...
# Pack into .tar.gz per Fabric v2.4 lifecycle format
(cd "${TMP_PKG}/connection" && tar czf "${TMP_PKG}/code.tar.gz" connection.json)
(cd "${TMP_PKG}" && tar czf "/tmp/${CC_LABEL}.tar.gz" metadata.json code.tar.gz)

CC_PACKAGE="/tmp/${CC_LABEL}.tar.gz"
log "Chaincode package ready: ${CC_PACKAGE}"

# ─── 3. Install on all peers ──────────────────────────────────────────────────
log "Installing chaincode on peer0.firma..."
docker exec fabric-cli peer lifecycle chaincode install \
  /opt/gopath/src/github.com/chaincode/legalcc_pkg.tar.gz 2>/dev/null || \
docker cp "${CC_PACKAGE}" fabric-cli:/opt/gopath/src/github.com/chaincode/legalcc_pkg.tar.gz && \
docker exec fabric-cli peer lifecycle chaincode install \
  /opt/gopath/src/github.com/chaincode/legalcc_pkg.tar.gz
...
docker cp "${CC_PACKAGE}" fabric-cli:/tmp/legalcc_pkg.tar.gz
docker exec \
  ...
  fabric-cli peer lifecycle chaincode install /tmp/legalcc_pkg.tar.gz
...
# ─── 7. Start chaincode server container ──────────────────────────────────────
...
log ""
log "════════════════════════════════════════════"
log "  legalcc chaincode deployed on legal-channel"
log "  Package ID: ${PACKAGE_ID}"
log "════════════════════════════════════════════"
